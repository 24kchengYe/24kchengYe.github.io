...
Responsible for **Spec–Refine data synthesis**, Judge/Fix Infra Models, Verifier Agent + AutoResearch, and Spec–Verifier–Code co-training for Lark Base AI scenarios.

**[1] Built the Spec–Refine data-synthesis flywheel.** To **break the capability ceiling of multi-dimensional table Spec generation in Base scenarios** and provide dense training signals for **reinforcement learning of a Refine agent based on Judge/Fix models**, built a three-tier **Category–Rubric–Mode** rubric system across Table/Workflow/Dashboard/Permission/View and created the **Judge–Fix–Router** pipeline. Combined multi-candidate generation, domain review/root-cause routing, local verification/long-term repair memory, voting, terminal bypass, format-gated release, and tolerance review/exemption, with Hard-path fallback for difficult chains; early stopping and mixed-precision inference controlled cost, while fault isolation/transaction rollback protected results. Raised **data accuracy from 54% to 95% (+41pp)** on E2E machine evaluation + LLPP human evaluation, connected online user data from Doubao Office scenarios, and formed an **Infra Model training data flywheel**.

**[2] Built Judge/Fix Infra Models.** To address **sparse Code Agent signals, insufficient hard-case coverage, and long-tail failures**, designed **curriculum RL** for the Lark Base Rubric Model and **post-trained a Seed 2.0-based Judge model** on 12,000+ synthetic data trajectories from the Judge–Fix pipeline. Cold-started with Strict-SFT, introduced **Category–Rubric–Mode tiered F1 rewards** into GRPO with **stage-wise dynamic weights**, built a confidence-centered structured reasoning chain, and used E2E Judge feedback to strengthen fine-grained rule-boundary mapping and false-positive control, with accuracy matching GPT-5.6; **vote agreement and rubric coverage exceeded it by 35pp and 45pp**. To stabilize Rubric-Model data repair, extracted Fix capabilities from the synthesis pipeline using heterogeneous multi-teacher trajectories and trained a **Fix Model** with SFT+RFT+PPO, **exceeding GPT-5.6 accuracy by 20pp**.
...
- 负责 **Spec-Refine 数据合成**、Judge/Fix Infra Models、Verifier Agent + AutoResearch 以及 Spec–Verifier–Code 联合训练，面向 Lark Base AI 场景提升企业级智能体能力。
- **[1] 构建 Spec-Refine 的数据合成飞轮。** **为突破 Base 场景多维表格生成Spec的能力上限**，并为**基于Judge、Fix 模型的Refine智能体的强化学习**提供**稠密训练信号**，构建覆盖 Table/Workflow/Dashboard/Permission/View 五大域的**Category–Rubric–Mode**三层 Rubric 体系与**Judge–Fix–Router**链路。通过多候选生成、分域评审/根因路由、局部复核/长期修复记忆，结合投票准出、终末旁路、格式准出、容差复核/豁免，难链路Hard兜底；以早停/混合精度控制开销，以异常隔离/事务回滚保护结果。E2E 机评与 LLPP 人评测试的**数据准确率由 54% 提升至 95%（+41pp）**，并接入豆包办公场景在线数据，形成**Infra Model 训练数据飞轮**。
- **[2] 构建 Judge/Fix Infra Models。** **针对 Code Agent 信号稀疏、难例覆盖不足和长尾问题**，为 Lark Base Rubric Model 设计**课程式强化学习**，基于 Judge–Fix 链路生成的 12,000+ 条合成数据轨迹**后训练 Seed 2.0-based Judge模型**。采用 Strict-SFT 冷启动，将**Category–Rubric–Mode 分层 F1 Reward**引入 GRPO，以**阶段性动态权重递进**构建置信度结构化思维链，并用 E2E Judge 强化细粒度规则边界映射和误报控制，准确率与 GPT-5.6 持平，**Vote 一致率和 Rubric 覆盖率分别提升 35pp 和 45pp**。为稳定 Rubric Model 数据修复，利用异构多教师轨迹抽取合成链路 Fix 能力，通过 SFT+RFT+PPO 训练 **Fix Model**，**准确率较 GPT-5.6 提升 20pp**。
...
- **Spec–Refineデータ合成**、Judge/Fix Infra Model、Verifier Agent + AutoResearch、Spec–Verifier–Code共同訓練を担当し、Lark Base AIシナリオの企業向けエージェント能力を向上。
- **[1] Spec–Refineデータ合成フライホイールを構築。** Baseシナリオの多次元テーブルSpec生成能力の上限を突破し、**Judge/Fixモデルに基づくRefineエージェントの強化学習**へ**稠密な訓練信号**を提供するため、**Category–Rubric–Mode**三層ルーブリック体系と**Judge–Fix–Router**パイプラインを構築。多候補生成、ドメインレビュー/根因ルーティング、局所検証/長期修復メモリ、投票、ターミナルバイパス、フォーマットゲート、許容差レビュー/免除を組み合わせ、難しいチェーンにはHard-path fallbackを適用；早期停止と混合精度推論でコストを制御し、結果を保護。E2E機械評価とLLPP人手評価で**データ精度を54%から95%へ（+41pp）**向上させ、豆包オフィスのオンラインデータを接続し、**Infra Model訓練データフライホイール**を形成。
- **[2] Judge/Fix Infra Modelを構築。** **Code Agentの信号の疎さ、難例カバレッジ不足、ロングテール問題に対し**、Lark Base Rubric Model向けに**カリキュラム強化学習**を設計し、Judge–Fixパイプラインの12,000+件の合成データ軌跡で**Seed 2.0-based Judgeモデルを後訓練**。Strict-SFTでコールドスタートし、**Category–Rubric–Mode階層F1 Reward**と**段階的動的重み**をGRPOへ導入。E2E Judgeフィードバックで細粒度ルール境界と誤検知制御を強化し、精度はGPT-5.6と同等、Vote一致率とRubricカバレッジを**35pp、45pp**向上。合成チェーンのFix能力を抽出し、SFT+RFT+PPOで**Fix Model**を訓練、**GPT-5.6比で精度20pp向上**。
