...
Conducted post-training of **Doubao Seed 2** for Lark Base AI scenarios, spanning Spec generation, Verifier repair, CodeAct generation, and the E2E Judge/Reward system to build domain-specific agent capabilities for enterprise-native B2B workflows.
- Built a **Judge Infra Model** with a Category–Rubric–Mode hierarchy covering five core domains (Table/Workflow/Dashboard/Permission/Form), severity levels S0–S1–S2, and semantic checker levels L1–L2–L3. Constructed a Judge–Fix–Router pipeline with voting, early stopping, bypass, and information passing, and synthesized judge trajectories. Post-trained the Judge Model on Seed 2.0 with curriculum reinforcement learning: Strict-SFT warm start followed by same-query multi-rollout GRPO, stage-wise dynamic Rubric:Mode weighting, Category→Rubric→Mode→Reason structured reasoning, and multidimensional E2E Judge signals. Its accuracy matched GPT-5.6 while **vote agreement improved by 35% and rubric coverage by 45%**.
- Built a **Fix Model** from **12,000 Judge–Fix and E2E trajectories**, distilling fix capabilities from heterogeneous advanced models across the Judge→Fix→Router stages. Applied SFT, RFT, and PPO on Seed 2.0; E2E feedback evaluation showed **20% higher accuracy than GPT-5.6**.
- Built a **Verifier Agent** that replaces sparse LLM-as-a-Judge verdicts with dense fix–judge feedback extracted from token-level multiband log probabilities before and after fixes through K-rollout sampling. Combined early stopping, V/Q voting, and mixed-precision inference to improve **token efficiency by 58%**; increased **E2E human–model agreement by 60%**, synthesized **11,000+ training trajectories**, and retained **9,197 validated samples**.
- Co-trained **Spec/Plan, CodeAct, and Judge/Reward** models. Trained a business Judge Model from accumulated Spec/Verifier annotations, Fix trajectories, and E2E outcomes, converting requirement coverage, configuration correctness, cross-module dependencies, and overall usability into rewards for CodeAct RL to jointly improve requirement understanding, verification and repair, and end-to-end execution.
- Developed **AutoResearch**, a self-evolving dynamic harness for prompt-engineering optimization in Base Spec generation. It performs multi-rollout sampling and cross-batch adversarial synthesis to suppress sampling noise and identify shared root causes, then applies evidence, stability, adversarial-robustness, and net-gain gates to produce high-leverage, low-regression prompt patches.
