...
- **[4] Spec–Verifier–Code 联合训练。** 推进 Spec/Plan、CodeAct 与 Judge/Reward 多模型联合训练，构建 Spec–Verifier–Code 联合训练与 AutoResearch 闭环：利用 Spec/Verifier 标注、Fix 轨迹和 E2E 结果训练业务 Judge Model，将需求覆盖、配置正确性、跨模块依赖和整体可用性等反馈转化为**CodeAct RL Reward**，协同提升需求理解、验证修复与端到端执行成功率。P50 延迟由**3.1 分钟降至 23 秒**，支撑规模化上线。
...
- **[1] Spec–Refineデータ合成フライホイールを構築。** Baseシナリオの多次元テーブル生成能力の上限を突破し、**Judge/Fixモデルに基づくRefineエージェントの強化学習**へ**稠密な訓練信号**を提供するため、**Category–Rubric–Mode**三層ルーブリック体系と**Judge–Fix–Router**パイプラインを構築。多候補生成、ドメインレビュー/根因ルーティング、局所検証/長期修復メモリ、投票、ターミナルバイパス、フォーマットゲート、許容差レビュー/免除を組み合わせ、難しいチェーンにはHard-path fallbackを適用；早期停止と混合精度推論でコストを制御し、結果を保護。E2E機械評価とLLPP人手評価で**データ精度を54%から95%へ（+41pp）**向上させ、豆包オフィスのオンラインデータを接続し、**Infra Model訓練データフライホイール**を形成。
...
- **[3] Verifier Agent + AutoResearchを構築。** Fix前後の局所Rubric判定から**多段階token確率（logprob）**を収集し、多路rollout（K）でFix–Judgeの稠密フィードバックを抽出。**SkillOpt 1\&2**とSeed 2.0 Liteを基にBase Spec生成向け自己進化PE最適化Harnessを改造し、同一Promptベースライン、多路rollout、クロスバッチ敵対的統合、共通根因発見、各種ゲートによる低回帰Prompt patch合成を実装。**token効率58pp、E2E人間・機械一致率60pp**を向上し、**9,197件の有効データ**を蓄積。
- **[4] Spec–Verifier–Code共同訓練。** Spec/Plan、CodeAct、Judge/Rewardの多モデル共同訓練を推進し、Spec/Verifierアノテーション、Fix軌跡、E2E結果を**CodeAct RL Reward**へ変換して要件理解・検証修復・端到端実行成功率を向上。P50遅延を**3.1分から23秒**へ短縮し、スケール展開を支援。
