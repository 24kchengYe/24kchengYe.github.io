...
**[4] Spec–Verifier–Code co-training.** To overcome **Ark model unit-resource limits** and **online product response-speed constraints**, pursued joint training of Spec/Plan, CodeAct, and Judge/Reward models, converting Spec/Verifier annotations, Fix trajectories, and E2E outcomes into **CodeAct RL rewards** to improve requirement understanding, verification and repair, and end-to-end execution success. P50 latency fell from **3.1 min to 23 s**, supporting scale-up.
...
- **[1] 构建 Spec-Refine 的数据合成飞轮。** **为突破 Base 场景多维表格生成Spec的能力上限**，并为**基于Judge、Fix 模型的Refine智能体的强化学习**提供**稠密训练信号**，构建覆盖 Table/Workflow/Dashboard/Permission/View 五大域的**Category–Rubric–Mode**三层 Rubric 体系与**Judge–Fix–Router**链路。通过多候选生成、分域评审/根因路由、局部复核/长期修复记忆，结合投票准出、终末旁路、格式准出、容差复核/豁免，难链路Hard兜底；以早停/混合精度控制开销，以异常隔离/事务回滚保护结果。E2E 机评/人评显示**数据准确率由 54% 提升至 95%（+41pp）**，并接入豆包办公场景数据，形成**Infra Model 训练数据飞轮**。
- **[2] 构建 Judge/Fix Infra Models。** **针对 Code Agent 信号稀疏、难例覆盖不足和长尾问题**，为 Spec Rubric Model 设计**课程式强化学习**，基于 Judge–Fix 链路生成的 1万+ 条合成数据轨迹**后训练 Seed 2.0-based Judge模型**。采用 Strict-SFT 冷启动，将**Category–Rubric–Mode 分层 F1 Reward**引入 GRPO，以**阶段性动态权重递进**构建置信度结构化思维链，并用 E2E Judge 强化细粒度规则边界映射和误报控制，准确率与 GPT-5.6 持平，**Vote 一致率和 Rubric 覆盖率分别提升 35pp 和 45pp**。为提升Fix效率，利用异构多教师轨迹抽取合成链路 Fix 能力，通过 SFT+RFT+PPO 训练 **Fix Model**，**准确率较 GPT-5.6 提升 20pp**。
...
- **[4] Spec–Verifier–Code 联合训练。** **为突破方舟模型单元资源上限**、**解决产品线上速度问题**，推进 Spec/Plan、CodeAct 与 Judge/Reward 多模型联合训练，构建 Spec–Verifier–Code 联合训练与 AutoResearch 闭环：利用 Spec/Verifier 标注、Fix 轨迹和 E2E 结果训练业务 Judge Model，将需求覆盖、配置正确性、跨模块依赖和整体可用性等反馈转化为**CodeAct RL Reward**，协同提升需求理解、验证修复与端到端执行成功率。P50 延迟由**3.1 分钟降至 23 秒**，支撑规模化上线。
...
- **[1] Spec–Refineデータ合成フライホイールを構築。** Baseシナリオの多次元テーブルSpec生成能力の上限を突破し、**Judge/Fixモデルに基づくRefineエージェントの強化学習**へ**稠密な訓練信号**を提供するため、**Category–Rubric–Mode**三層ルーブリック体系と**Judge–Fix–Router**パイプラインを構築。多候補生成、ドメインレビュー/根因ルーティング、局所検証/長期修復メモリ、投票、ターミナルバイパス、フォーマットゲート、許容差レビュー/免除を組み合わせ、難しいチェーンにはHard-path fallbackを適用；早期停止と混合精度推論でコストを制御し、結果を保護。E2E機械/人手評価で**データ精度を54%から95%へ（+41pp）**向上させ、豆包オフィスのシナリオデータを接続し、**Infra Model訓練データフライホイール**を形成。
- **[2] Judge/Fix Infra Modelを構築。** **Code Agentの信号の疎さ、難例カバレッジ不足、ロングテール問題に対し**、Spec Rubric Model向けに**カリキュラム強化学習**を設計し、Judge–Fixパイプラインの1万+件の合成データ軌跡で**Seed 2.0-based Judgeモデルを後訓練**。Strict-SFTでコールドスタートし、**Category–Rubric–Mode階層F1 Reward**と**段階的動的重み**をGRPOへ導入。E2E Judgeフィードバックで細粒度ルール境界と誤検知制御を強化し、精度はGPT-5.6と同等、Vote一致率とRubricカバレッジを**35pp、45pp**向上。Fix効率を高めるため、合成チェーンのFix能力を抽出し、SFT+RFT+PPOで**Fix Model**を訓練、**GPT-5.6比で精度20pp向上**。
...
- **[4] Spec–Verifier–Code共同訓練。** **方舟モデルの単位リソース上限を突破し**、**プロダクトオンラインの速度問題を解決するため**、Spec/Plan、CodeAct、Judge/Rewardの多モデル共同訓練を推進し、Spec/Verifierアノテーション、Fix軌跡、E2E結果を**CodeAct RL Reward**へ変換して要件理解・検証修復・端到端実行成功率を向上。P50遅延を**3.1分から23秒**へ短縮し、スケール展開を支援。
